...
class WeiboController extends AdminController
{
    ...
    public function topic($page = 1, $r = 20, $content = '')
    {
        //读取微博列表
        $model = M('Topic');
        if ($content != '')
        ...
        $builder->title('话题管理')
       		->setStatusUrl(U('setTopicStatus'))->buttonDelete()->button('推荐', $attr1)->button('取消推荐', $attr0)
            ->keyId()->keyText('name','话题')->keyUid()->keyCreateTime()->keyStatus()
            ->keyMap('is_top', '推荐', array(0 => '不推荐', 1 => '推荐'))
            ->search('内容', 'content')
            ->data($list)
            ->pagination($totalCount, $r)
            ->display();
    ...
    public function setTopicStatus($ids, $status)
    {
    	$builder = new AdminListBuilder();
    	$builder->doSetStatus('Topic', $ids, $status);
    }
    
    public function setTopicTop($ids, $top)
    {
        foreach ($ids as $id) {
            D('Topic')->where(array('id' => $id))->setField('is_top', $top);
            S('topic_' . $id, null);
        }
