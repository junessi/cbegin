 ...
 * @return array
 */
function get_district_id_array(){
	$districts = M('District')->where(array('level' => 1))->select();
	foreach ($districts as $d){
		$district_id_array[$d['id']] = $d['name'];
	}
	$district_id_array[2] = '海外';
	$district_id_array[1] = '其他';
	$district_id_array[0] = '请选择区域';
	
	return $district_id_array;
}


function get_user_avatar($uid){
	$path = M('Avatar')->where('uid='.$uid)->find();
	if ($path){
		return "/Uploads/Avatar/".$path['path'];
	}else{
		return "/Addons/Avatar/default_64_64.jpg";
	}
}
