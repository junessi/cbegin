...
function query_user($fields = null, $uid = null)
{
    ...
    //发布项目数，关注项目数 ，评论项目数
    if (in_array('project_count', $fields)){
    	$result['project_count'] = D('Document')->where('uid='.$uid.' and category_id in (51,52)')->count();
    }
    if (in_array('project_collection', $fields)){
    	$project_ids = D('DocumentProject')->getField('id',true);
    	$project_ids = implode(",", $project_ids);
    	$result['project_collection'] = D('DocumentCollection')->where("uid=".$uid." and document_id in (".$project_ids.")")->count();
    }
    if (in_array('project_comment', $fields)){
    	$project_ids = D('DocumentProject')->getField('id',true);
    	$project_ids = implode(",", $project_ids);
    	$result['project_comment'] = D('LocalComment')->where("uid=".$uid." and row_id in (".$project_ids.")")->count();
    }
    
    //发布资讯，收藏资讯，评论资讯数
    if (in_array('news_count', $fields)){
    	$category_ids = D('Home/DocCategory')->getChildrenId(1);
    	$result['news_count'] = D('Document')->where('uid='.$uid.' and category_id in ('.$category_ids.')')->count();
    }
    
    if (in_array('news_collection', $fields)){
    	$news_ids = D('DocumentArticle')->getField('id',true);
    	$news_ids = implode(",", $news_ids);
    	$result['news_collection'] = D('DocumentCollection')->where("uid=".$uid." and document_id in (".$news_ids.")")->count();
    }
    if (in_array('news_comment', $fields)){
    	$news_ids = D('DocumentArticle')->getField('id',true);
    	$news_ids = implode(",", $news_ids);
    	$result['news_comment'] = D('LocalComment')->where("uid=".$uid." and row_id in (".$news_ids.")")->count();
    }
