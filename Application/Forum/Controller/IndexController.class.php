...
class IndexController extends Controller
{
    ...
    public function index($page = 1)
    {
        ...
        $Forum = D('Forum');
       	...
       	$types = $Forum->getAllForumsSortByTypes();
    ...
    public function doZan(){
    	$id = $_POST['id'];
    	$result = D('Forum')->where("id=".$id)->setInc('zan',1);
    	if ($result){
    		$this->success('点赞成功');
    	}
    	else{
    ...
    public function doFollow(){
    	if (IS_AJAX){
    		if (!is_login()){
    			$this->error('登陆后才可以关注项目');
    		}
    		else {
    			$map['uid'] = is_login();
    			$map['forum_id'] = $_POST['id'];
    			 
    			$rs = M('ForumFollow')->where($map)->find();
    			...
    			if ($rs){
    				...
    				$res = M('ForumFollow')->add($data);
    	
    				if ($res){
    					M('Forum')->where('id='.$_POST['id'])->setInc('follow',1);
    					$this->success('关注成功');
    				}
    				else{
    					$this->error('关注失败');
