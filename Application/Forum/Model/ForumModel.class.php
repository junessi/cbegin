...
class ForumModel extends Model
{
    ...
    public  function getForumList($map_type = array('status' => 1) ,$order = '')
    {
        $tag='forum_list_'.serialize($map_type);
        //$forum_list = S($tag);
        $cache_time=modC('CACHE_TIME',100,'Forum');
        if (empty($forum_list)) {
            //读取板块列表
           if ($order == ''){
           		$order = 'sort asc';
            ...
            $forum_list = D('Forum/Forum')->where($map_type)->order($order)->select();
