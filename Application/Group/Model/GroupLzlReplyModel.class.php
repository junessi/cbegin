...
class GroupLzlReplyModel extends Model
{
    ...
    public function addLZLReply($post_id, $to_f_reply_id, $to_reply_id, $to_uid, $content,$p, $send_message = true)
    {
        ...
        action_log('add_post_reply', 'GroupLzlReply', $result, is_login());
    ...
    private function sendReplyMessage($uid, $post_id, $content, $to_uid, $to_f_reply_id,$result,$p)
    {
        ...
        $count = D('GroupLzlReply')->where($map)->count();
        ...
        $url = U('Group/Index/detail', array('id' => $post_id,'page'=>$p,'sr'=>$to_f_reply_id,'sp'=>$pageCount)).'#'.$to_f_reply_id;
        $from_uid = $uid;
        $type = 2;
    ...
    public function getLZLReplyList($to_f_reply_id, $order, $page=1, $limit)
    {
        ...
        if ($list == null) {
            $list = D('group_lzl_reply')->where('is_del=0 and to_f_reply_id=' . $to_f_reply_id)->order($order)->select();
            foreach ($list as $k => &$v) {
