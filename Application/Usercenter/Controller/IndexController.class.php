...
class IndexController extends BaseController
{
    ...
    public function index($uid = null, $page = 1, $count = 10)
    {
        ...
        $this->setKeywords($str . "，个人主页，Cbegin,创始网，个人" . $str_app);
    ...
    public function information($uid = null, $info_type = "base_info")
    {
        ...
        if ($info_type == "base_info") {
            //调用API获取基本信息
            //TODO tox 获取省市区数据
            $user = query_user(array('nickname', 'signature', 'email', 'mobile', 'rank_link', 'sex', 'pos_province', 'pos_city', 'pos_district', 'pos_community','home_province','home_city','home_dustrict','focus'), $uid);
            ...
            if ($user['home_province'] != 0){
            	
            	$user['home_province'] = D('district')->where(array('id' => $user['home_province']))->getField('name');
            	$user['home_city'] = D('district')->where(array('id' => $user['home_city']))->getField('name');
            	$user['home_district'] = D('district')->where(array('id' => $user['home_district']))->getField('name');
            	
            }
            
            if ($user['focus']){
            	$map['id'] = array('in',$user['focus']);
            	$arrFocus = M('ProjectTags')->where($map)->field('title')->select();
            	
            	$user['focus'] = $arrFocus;
            	$this->assign('myFocus',$arrFocus);
            }
            //显示页面
        ...
        $this->setKeywords($str . "，个人资料，Cbegin,创始网");
     ...
     * 获取用户创业集结相关信息
     */
    public function project($uid=null,$info_type = 'release'){
   		$info_type = op_t($info_type);
   		...
   		if ($info_type == 'release'){
   			//发布项目
   			$project_ids = D('DocumentProject')->getField('id',true);
   			$project_ids = implode(",", $project_ids);
   			$map['id'] = array('in',$project_ids);
   			$map['uid'] = $uid;
   			
   			$Document = D('Document');
   			$lists = $Document->where($map)->order('create_time desc')->page($p,$list_row)->select();
   		...
   		elseif ($info_type == 'collection'){
   			//关注的项目
   			$project_ids = D('DocumentProject')->getField('id',true);
   			$project_ids = implode(",", $project_ids);
   			$map['uid'] = $uid;
   			$map['document_id'] = array("in",$project_ids);
   			
   			$project_collections = D('DocumentCollection')->where($map)->order('create_time desc')->field('document_id,create_time')->select();
   			foreach ($project_collections as $pc){
   				$project_collection_ids[] = $pc['document_id'];
   				$project_collection_time[$pc['document_id']] = $pc['create_time'];//关注时间
   			}
   			$project_collection_ids = implode(",", $project_collection_ids);
   			unset($map['uid']);
   			unset($map['document_id']);
   			$map['id'] = array('in' , $project_collection_ids);
   			
   			
   			$Document = D('Document');
   			$lists = $Document->where($map)->page($p,$list_row)->select();
   			foreach ($lists as $v){
   				$v['collection_time'] = $project_collection_time[$v['id']];
   				$list[] = $v;
   			}
   		...
   		elseif ($info_type == 'comment'){
   			//评论的项目
   			
   			$table = C('DB_PREFIX').'local_comment a,'.C('DB_PREFIX').'document b';
   			$where = "a.row_id = b.id and a.mod='document' and b.category_id in (51,52) and a.uid=".$uid;
   			$fields = "a.* ,b.title";
   			$lists = M()->table($table)->where($where)->order('a.create_time desc')->page($p,$list_row)->field($fields)->select();
   		...
   		$str = '{$user_info.nickname|op_t}';
   		$this->setTitle($str . "的创业集结项目");
   		$this->setKeywords($str . "，创业集结，Cbegin,创始网");
   		$this->setDescription($str . "的个人创业集结页");
   		//四处一词 seo end
    ...
    public function news($uid=null,$info_type = 'release'){
    	$info_type = op_t($info_type);
    	...
    	if($info_type == 'release'){
    		...
    		$CategoryModel = D('DocCategory');
    		$category_ids = $CategoryModel->getChildrenId(1);
    		$Document = D('Document');
    		$where = "uid=".$uid." and category_id in (".$category_ids.")";
    		$fields = "id,uid,category_id,title,description,create_time,status";
    		$lists = $Document->where($where)->order('create_time desc')->page($p,$list_row)->field($fields)->select();
    	...
    	elseif ($info_type == 'collection'){
    		$news_ids = D('DocumentArticle')->getField('id',true);
    		$news_ids = implode(",", $news_ids);
    		$map['uid'] = $uid;
    		$map['document_id'] = array("in",$news_ids);
    		
    		$news_collections = D('DocumentCollection')->where($map)->order('create_time desc')->field('document_id,create_time')->select();
    		foreach ($news_collections as $nc){
    			$news_collection_ids[] = $nc['document_id'];
    			$news_collection_time[$nc['document_id']] = $nc['create_time'];//关注时间
    		}
    		$news_collection_ids = implode(",", $news_collection_ids);
    		unset($map['uid']);
    		unset($map['document_id']);
    		$map['id'] = array('in' , $news_collection_ids);
    		
    		
    		$Document = D('Document');
    		$lists = $Document->where($map)->page($p,$list_row)->select();
    		foreach ($lists as $v){
    			$v['collection_time'] = $news_collection_time[$v['id']];
    			$list[] = $v;
    		}
    	...
    	elseif ($info_type == 'comment'){
    		...
    		$table = C('DB_PREFIX').'local_comment a,'.C('DB_PREFIX').'document b';
    		$where = "a.row_id = b.id and a.mod='document' and b.category_id in (".$category_ids.") and a.uid=".$uid;
    		$fields = "a.* ,b.title";
    		$lists = M()->table($table)->where($where)->order('a.create_time desc')->page($p,$list_row)->field($fields)->select();
    	...
    	$str = '{$user_info.nickname|op_t}';
    	$this->setTitle($str . "的资讯信息");
    	$this->setKeywords($str . "，资讯，Cbegin,创始网");
    	$this->setDescription($str . "的个人资讯页");
    	//四处一词 seo end
    ...
    public function fans($uid = null, $page = 1)
    {
        ...
        $this->setKeywords($str . "，个人粉丝，Cbegin,创始网");
    ...
    public function following($uid = null, $page = 1)
    {
        ...
        $this->setKeywords($str . "，个人关注，Cbegin,创始网");
    ...
    public function rank($uid = null)
    {
        ...
        $this->setKeywords($str . "，个人头衔，Cbegin,创始网");
    ...
    public function rankVerify($rank_user_id = null)
    {
        ...
        $this->setKeywords($str . "，个人头衔，头衔申请，Cbegin,创始网");
