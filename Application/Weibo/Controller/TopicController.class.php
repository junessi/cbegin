...
class TopicController extends Controller
{
    ...
    public function index($topk = '', $uid = 0, $lastId = 0)
    {
        ...
        if ($topic['status'] != 1){
        	$this->error('话题已关闭或者删除', U('Weibo/Index/index'));
        }
        ...
        //关注
        $mapFollow['uid'] = is_login();
        $mapFollow['topic_id'] = $topic['id'];
		$is_follow = M('TopicSub')->where($mapFollow)->find();
		if ($is_follow){
			$this->assign('is_follow',true);
		}else{
			$this->assign('is_follow',false);
			
		}
		
		//关注人数
		$topic_sub = M('TopicSub')->where("topic_id=".$topic['id'])->count();
		$this->assign('total_sub',$topic_sub);
    ...
    public function follow(){
    	$data['topic_id'] = $topic_id = $_POST['topic_id'];
    	$data['uid'] = is_login();	
    	$is_have = M('TopicSub')->where($data)->find();
    	if ($is_have){
    		$this->error('您已近关注过');
    	}else{
    		$data['create_time'] = time();
    		$rs = M('TopicSub')->add($data);
    		if ($rs){
    			$this->success('关注话题成功');
    		}else{
    			$this->error('关注话题失败');
