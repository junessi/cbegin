...
<div id="main-container" class="container">
  ...
  <?php if(is_array($categoryNav)): foreach($categoryNav as $key=>$vo): ?><li role="presentation" <?php if(($currentCate["id"]) == $vo['id']): ?>class="active"<?php endif; ?> ><a href="/news/<?php echo ($vo['name']); ?>/"><?php echo ($vo["title"]); ?></a></li><?php endforeach; endif; ?>
			...
			<div class="row">
				<div class="col-md-12">
					<h1 class="news-detail-title"><?php echo ($info["title"]); ?></h1>
					<div class="news-detail-info text-right">
					<p>
						<span>时间: <?php echo (date('Y-m-d',$info["create_time"])); ?></span>
						<span>阅读量: <?php echo ($info["view"]); ?></span>
						<span>发布者: <a href="<?php echo U('usercenter/index/index?uid='.$info['uid']);?>" ><?php echo (get_username($info["uid"])); ?></a></span>
					</p>
					</div>
					<?php if(!empty($info["cover_id"])): ?><div class="news-detail-img"><img src="<?php echo (get_cover($info["cover_id"],'path')); ?>" width="100%"/></div><?php endif; ?>
					<div class="news-detail-content"><?php echo ($info["content"]); ?></div>
					<div class="news-detail-original">
						<span ><?php echo ($info['is_original'] ? '原创' : '转载'); ?> <?php echo ($info["source_url"]); ?></span>
					</div>
					<div class="news-detail-social row">
						...
						<div class="col-md-6 text-right">
							<button class="btn btn-zan do-zan" data-id="<?php echo ($info["id"]); ?>" data-zan="0">点赞 <em><?php echo ($info["zan"]); ?></em></button>
							<button class="btn btn-collection do-collection" data-id="<?php echo ($info["id"]); ?>">收藏 <em><?php echo ($info['collection'] ? $info['collection'] : 0); ?></em></button>
						</div>
					</div>
					<div class="news-detail-tuijian">
						<div id="hm_t_55499"></div>
					</div>
					<div class="news-detail-comment" id="comment">
					<?php echo hook('localComment',array('path'=>'home/document/'.$info['id'],'uid'=>$uid));?>
					</div>
				</div>
