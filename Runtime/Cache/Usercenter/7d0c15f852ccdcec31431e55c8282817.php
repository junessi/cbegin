...
<div class="row ">
    ...
    <?php $user = query_user(array('message')); ?>
    ...
    <div class="row">
        <div class="col-xs-12">
            ...
            <ul class="nav nav-pills ucenter-tab">
                ...
                ><a href="<?php echo U('Usercenter/Message/session',array('tab'=>'session'));?>">所有聊天</a></li>
            </ul>
    ...
    <div class="row talk_list">
        <div id="scrollContainer">
            <?php if(is_array($talks)): $i = 0; $__LIST__ = $talks;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$talk): $mod = ($i % 2 );++$i;?><div class="row talk_line">
                    <div class="col-xs-2">
                        <div class="row heads text-center">
                            <?php if(is_array($talk["users"])): $i = 0; $__LIST__ = $talk["users"];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$user): $mod = ($i % 2 );++$i; if(count($talk['users']) != 2): ?><div class="pull-left" style="margin:11px;margin-top: 0">
                                        <div><img style="width:32px" class="avatar-img" ucard="<?php echo ($user["id"]); ?>"
                                                  src="<?php echo ($user["avatar64"]); ?>"></div>
                                    </div>
                                    <?php else: ?>
                                    <div class="pull-left" style="margin:25px;margin-top: 0">
                                        <div>
                                            <img style="width:64px" class="avatar-img" ucard="<?php echo ($user["id"]); ?>"
                    ...
                    <div class="col-xs-10">
                        <div class="pull-right">
                            <a class="btn btn-danger ajax-post"
                               href="<?php echo U('Usercenter/Message/doDeleteTalk', array('talk_id'=>$talk['id']));?>">
                                <span class="glyphicon glyphicon-remove"></span> 关闭
                            </a>
                        </div>
                        <h2>
                            <a href="<?php echo U('UserCenter/Message/talk',array('talk_id'=>$talk['id']));?>">
                                <?php echo (op_t($talk["title"])); ?>&nbsp;（<?php echo count($talk['users']);?>人）
                            </a>
                        </h2>

                        <?php if($talk['last_message']['content'] != ''): ?><p>
                                <a href="<?php echo ($talk["last_message"]["user"]["space_url"]); ?>" ucard="<?php echo ($talk["last_message"]["user"]["id"]); ?>">
                                    <?php echo (op_t($talk["last_message"]["user"]["username"])); ?>
                                </a>： &nbsp;&nbsp;<?php echo ($talk["last_message"]["content"]); ?> &nbsp; &nbsp; &nbsp;<?php echo (friendlydate($talk["last_message"]["create_time"])); ?>
                            </p>
                            <?php else: ?>
                            <p>暂无对话内容</p><?php endif; ?>
                    </div>
                </div>
                <hr/><?php endforeach; endif; else: echo "" ;endif; ?>
            <?php if(!$talks): ?><p style="font-size:3em;padding:2em 0;" class="text-muted text-center">暂无内容</p><?php endif; ?>
        </div>
    </div>
    <style>
        .talk_list {
    ...
    <script>
        ...
        var MID = "<?php echo ($self["id"]); ?>";
        var message_id = "<?php echo ($message_id); ?>";
        var talk_id = "<?php echo ($talk["id"]); ?>";
        var myhead = "<?php echo ($self["avatar128"]); ?>";
